...
---
### Redirection des comptes ordinateurs

Après la mise en place de la structure des Unités d’Organisation (OU) pour les sites de Paris et Lyon, la commande `redircmp` permet de rediriger automatiquement la création des comptes ordinateurs vers l’OU souhaitée.

Sans cette redirection, les ordinateurs qui rejoignent le domaine seraient placés dans l’OU par défaut de l’Active Directory.
Cela garantit que les nouveaux postes sont créés dans la bonne OU et qu’ils reçoivent immédiatement les **GPO** correspondant à leur emplacement.

#### Mise en œuvre avec redircmp et redirusr
